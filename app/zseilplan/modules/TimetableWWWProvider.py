...
class TimetableWWWProvider:
    ...
    def import_timesteps(self):
        ...
        for unit in self._units:
            ...
            for time_block in time_blocks:
                timesteps += time_block.innerText.split("-")
    ...
    def add_item_to_teacher_timetable(self, item: TimetableItem):
        ...
        if item.unit not in self.timetable_data.teacher_timetable[item.teacher][item.day][item.hour]:
            self.timetable_data.teacher_timetable[item.teacher][item.day][item.hour][item.unit] = []
        
        self.timetable_data.teacher_timetable[item.teacher][item.day][item.hour][item.unit].append(item.serialize())
        return True
    ...
    def import_timetable_for_unit(self, unit_name, unit_url):
        ...
        for hour,row in enumerate(rows):
            ...
            for column in columns:
                ...
                zseilplan.modules.utils.debug("Dzien {} - {} godzina lekcyjna".format(day, hour), level=2)
                
                '''TODO: make this a func'''

                # If this is the first iteration of this day, create dict
                if day not in self.timetable_data.timetable:
                    self.timetable_data.timetable[day] = {}
                
                # If this is the first iteration of this lesson in this day, create dict
                if hour not in self.timetable_data.timetable[day]:
                    self.timetable_data.timetable[day][hour] = {}
            
                # If this is the first iteration of this unit lesson in this day, create array
                if unit_name not in self.timetable_data.timetable[day][hour]:
                    self.timetable_data.timetable[day][hour][unit_name] = []
                ...
                for e in entries:
                    ...
                    if entry.getElementsByTagName("span")[0].className == "p":
                        ...
                        item = RawTimetableItem(unit = unit_name, 
                                                day = day, 
                                                hour = hour, 
                                                html_item = entry)
                        self.add_item_to_timetable(item.parse())
                        ...
                        for parent in entry.getElementsByTagName("span"):
    ...
    def do_things(self):
        ...
        with open("test.json", "w", encoding = "UTF-8") as f:
            ...
            output['timesteps'] = {"default": self.timetable_data.auto_timesteps[0][0]}
            ...
            output["teachermap"] = {"Testowy Test": "Testowy Test Test"}
